...
public class DungeonGenerator : MonoBehaviour
{
    ...
    {
        // 1. Limpiamos mapas por si acaso
        floorTilemap.ClearAllTiles();
        wallTilemap.ClearAllTiles();
        floorPositions.Clear();

        // 2. Ejecutamos el Random Walk para generar el suelo
        RunRandomWalk();

        // 3. Dibujamos los tiles visualmente
        DrawFloorTiles();

        // 4. Calculamos y dibujamos las paredes
        CreateWalls();

        // 5. Spawneamos al jugador en la primera casilla real de suelo
        if (playerPrefab != null && floorPositions.Count > 0)
        {
            // Convertimos el HashSet a un Array para coger el primer elemento seguro
            Vector2Int[] floorArray = new Vector2Int[floorPositions.Count];
            floorPositions.CopyTo(floorArray);

            // Colocamos al jugador ahí
            Vector3 playerSpawnPos = new Vector3(floorArray[0].x, floorArray[0].y, 0f);
    ...
    private void SpawnEnemies()
    {
        ...
        // Quitamos la posición exacta donde hemos puesto al jugador para que no le caigan encima
        ...
        {
            if (availablePositions.Count == 0) break; // Prevención de errores si el mapa es minúsculo
            ...
            // Quitamos la posición elegida para no amontonar monstruos
